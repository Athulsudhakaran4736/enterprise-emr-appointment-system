import { Alert, Card, Empty, List, Pagination, Select, Spin, Tag, Typography } from 'antd'
...
function ReceptionistSchedulesPage() {
  ...
  const doctorOptions = useMemo(
    () => doctors.map((doctor) => ({ value: doctor.id || doctor._id, label: `${doctor.user?.name} - ${doctor.specialization}` })),
    [doctors],
  )
    ...
    <div className="space-y-6">
      <Card className="rounded-[28px] border-0 shadow-sm">
        <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <Title level={3} className="!mb-2">Doctor Schedules</Title>
            <Paragraph className="!mb-0 !text-slate-500">
              Read-only availability view for booking appointments and checking working hours.
            </Paragraph>
          </div>
          <Select
                ...
                >
                  <div className="mb-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    ...
                    <div>
                      ...
                      <div className="font-semibold text-slate-900">{schedule.workingDays.filter((day) => day.isWorking).length}</div>
                    </div>
                  </div>
                  ...
                  <List
                    dataSource={weekdayLabels.map((label, index) => ({
                      label,
                      entry: schedule.workingDays.find((day) => day.dayOfWeek === index),
                    }))}
